...
bool llama_context::set_sampler(llama_seq_id seq_id, llama_sampler * sampler) {
    ...
    // Backend sampling builds a ggml computation graph on the output device.
    // With SPLIT_MODE_TENSOR the output device is a meta device, and the
    // sampling graph creates intermediate tensors whose split state the meta
    // backend cannot determine (UNKNOWN), causing allocation to crash.
    // TODO: fix split state computation for sampling ops on meta devices.
